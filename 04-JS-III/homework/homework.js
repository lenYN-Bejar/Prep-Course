...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  if(array.indexOf(elemento) !== -1) return true
  return false
}
...
function numeroMasGrande(numeros) {
  ...
  for (let i = 0; i < numeros.length; i++) {
      if(numeros[i]>numeros[i+1]) {
          mayor = numeros[i]
          if(mayor>newnumber) newnumber = mayor
...
function todosIguales(arreglo) {
  ...
  let number = 1
  for (let i = 0; i < arreglo.length; i++) {
      if(arreglo[i] === arreglo[i+1]) number++
  }
  if(number === arreglo.length) return true
  else return false
} 
